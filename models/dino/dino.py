...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        features, poss = self.backbone(samples)  # (2, 512, 80, 88)  (2, 1024, 40, 44)  (2, 2048, 20, 22)
        ...
        if self.num_feature_levels > len(srcs):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    src = self.input_proj[l](features[-1].tensors)  # (2, 256, 10, 11)
                else:
                    src = self.input_proj[l](srcs[-1])
                m = samples.mask
                mask = F.interpolate(m[None].float(), size=src.shape[-2:]).to(torch.bool)[0]  # (2, 10, 11)
                pos_l = self.backbone[1](NestedTensor(src, mask)).to(src.dtype)  # (2, 256, 10, 11)
                srcs.append(src)
                masks.append(mask)
                poss.append(pos_l)
