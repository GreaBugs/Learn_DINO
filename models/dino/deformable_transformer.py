...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type == 'standard':
            ...
            # gather tgt
            # 同样的取出对应的memory
            tgt_undetach = torch.gather(output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model))  # torch.Size([2, 900, 256])

            # 对应于论文中图5 c的 static content queries
            if self.embed_init_tgt:  # True
                # [900,256] -> [900,bs,256] -> [bs,900,256], 这里依然使用的是网络的参数 一个mebedding
                tgt_ = self.tgt_embed.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1)  # nq, bs, d_model    torch.Size([2, 900, 256])
            else:
            ...
            if refpoint_embed is not None:  # torch.Size([2, 200, 4])
                # cat prepare_for_cdn生成的去噪的，加上match部分使用的
                refpoint_embed = torch.cat([refpoint_embed, refpoint_embed_], dim=1)  # torch.Size([2, 1100, 4])
                # cat prepare_for_cdn生成的去噪的，加上match部分使用的
                tgt = torch.cat([tgt, tgt_], dim=1)  # torch.Size([2, 1100, 256])
            else:
        ...
        hs, references = self.decoder(
            ...
            # 参考点位
            refpoints_unsigmoid=refpoint_embed.transpose(0, 1),  # torch.Size([2, 1100, 4])
            ...
            # prepare_for_cdn时生成的
            tgt_mask=attn_mask)  # torch.Size([1100, 1100])
        #########################################################
        # End Decoder
        ...
        if self.two_stage_type == 'standard':
            ...
            if self.two_stage_keep_all_tokens:  # False
                ...
                hs_enc = tgt_undetach.unsqueeze(0)  # torch.Size([2, 900, 256])
                # [1,bs,900,4] refpoint_embed_undetach是encoder的输出经过topk选取后的
                ref_enc = refpoint_embed_undetach.sigmoid().unsqueeze(0)  # torch.Size([1, 2, 900, 4])
        else:
            hs_enc = ref_enc = None
        #########################################################
        # End postprocess
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        """
        output = tgt  # torch.Size([1100, 2, 256])
        # 每一层decoder计算后的结果
        intermediate = []
        # [900+2*100, bs, 4] 限制在0-1
        reference_points = refpoints_unsigmoid.sigmoid()  # torch.Size([1100, 2, 4])
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.deformable_decoder:
                ...
                if reference_points.shape[-1] == 4:
                    ...
                    reference_points_input = reference_points[:, :, None] \
                                             * torch.cat([valid_ratios, valid_ratios], -1)[None, :]  # nq, bs, nlevel, 4  torch.Size([1100, 2, 4, 4])
                else:
                ...
                query_sine_embed = gen_sineembed_for_position(reference_points)  # nq, bs, 256*2  torch.Size([1100, 2, 512])

                reference_points_input = None

            # conditional query [N,bs,256]
            raw_query_pos = self.ref_point_head(query_sine_embed)  # nq, bs, 256  torch.Size([1100, 2, 256])

            pos_scale = self.query_scale(output) if self.query_scale is not None else 1
            # conditional detr的做法
            query_pos = pos_scale * raw_query_pos  # torch.Size([1100, 2, 256])
            # 如果不使用变形attention
            ...
            if not dropflag:
                # [N,bs,256]  torch.Size([1100, 2, 256])
                output = layer(
                    tgt=output,  # [N,bs,256]
            ...
            if self.bbox_embed is not None:
                # 得到特征图上的值
                reference_before_sigmoid = inverse_sigmoid(reference_points)  # torch.Size([1100, 2, 4])
                # 得到网络输出的修正值
                delta_unsig = self.bbox_embed[layer_id](output)  # torch.Size([1100, 2, 4])
                # 进行修正
                ...
                if self.use_detached_boxes_dec_out:
                    ...
                    # 这个是没有脱离的，这个地方的处理就是论文中的look forward twice
